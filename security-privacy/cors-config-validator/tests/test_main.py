"""Tests for main.py CLI and config.py."""
import os, sys, io, json, pytest
from unittest.mock import patch
import runpy

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from main import main, cmd_validate
from config import Config

def test_config(): assert Config is not None

def test_cmd_validate(tmp_path):
    f = tmp_path / "cors.json"
    f.write_text(json.dumps({"allow_origins": ["https://example.com"], "allow_methods": ["GET", "POST"]}))
    args = type('A', (), {'file': str(f)})()
    with patch("builtins.print") as p: cmd_validate(args)

def test_cmd_validate_stdin():
    args = type('A', (), {'file': '-'})()
    with patch("sys.stdin", io.StringIO(json.dumps({"allow_origins": ["*"]}))): 
        with patch("builtins.print") as p: cmd_validate(args)

def test_main_validate(tmp_path):
    f = tmp_path / "cors.json"
    f.write_text(json.dumps({"allow_origins": ["https://app.com"]}))
    with patch("sys.argv", ["main", "validate", str(f)]):
        with patch("builtins.print"): main()

def test_main_entry_point(tmp_path):
    f = tmp_path / "cors.json"
    f.write_text(json.dumps({"allow_origins": ["*"]}))
    with patch("sys.argv", ["main", "validate", str(f)]):
        with patch("builtins.print"):
            with patch.dict("sys.modules", {"__main__": None}):
                runpy.run_module("main", run_name="__main__", alter_sys=True)
