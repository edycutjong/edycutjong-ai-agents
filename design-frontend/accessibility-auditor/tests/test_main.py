"""Tests for main.py CLI and config.py."""
import os, sys, io, pytest
from unittest.mock import patch
import runpy

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from main import main, cmd_audit
from config import Config

def test_config(): assert Config is not None

def test_cmd_audit(tmp_path):
    f = tmp_path / "page.html"
    f.write_text("<html><head><title>Test</title></head><body><h1>Hello</h1></body></html>")
    args = type('A', (), {'file': str(f)})()
    with patch("builtins.print") as p: cmd_audit(args)

def test_cmd_audit_stdin():
    args = type('A', (), {'file': '-'})()
    with patch("sys.stdin", io.StringIO("<html lang=\"en\"><head><title>Test</title></head><body><h1>Hi</h1></body></html>")):
        with patch("builtins.print") as p: cmd_audit(args)

def test_main_audit(tmp_path):
    f = tmp_path / "page.html"
    f.write_text("<html><body><h1>Test</h1></body></html>")
    with patch("sys.argv", ["main", "audit", str(f)]):
        with patch("builtins.print"): main()

def test_main_entry_point(tmp_path):
    f = tmp_path / "page.html"
    f.write_text("<html><body>Test</body></html>")
    with patch("sys.argv", ["main", "audit", str(f)]):
        with patch("builtins.print"):
            with patch.dict("sys.modules", {"__main__": None}):
                runpy.run_module("main", run_name="__main__", alter_sys=True)
