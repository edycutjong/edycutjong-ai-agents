...
def test_check_ssl_expiry(mock_create_connection, mock_create_context):
    ...
    mock_context.wrap_socket.return_value.__enter__.return_value = Mock(
        getpeercert=lambda: {'notAfter': (datetime.utcnow().replace(year=datetime.utcnow().year + 1)).strftime('%b %d %H:%M:%S %Y GMT')}
    )
    ...
    # Test Exception
    mock_create_connection.side_effect = Exception("Socket Error")
    days, err = check_ssl_expiry("https://example.com")
    assert days is None
    assert "Socket Error" in err
...
def test_check_custom_regex(mock_get):
    ...
    match, error = check_custom_regex("https://example.com", "World")
    assert match is False
    assert "HTTP Status 404" in error

    mock_get.side_effect = Exception("Requests Error")
    match, err = check_custom_regex("https://example.com", "World")
    assert match is False
    assert "Requests Error" in err
