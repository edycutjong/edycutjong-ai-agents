#!/usr/bin/env python3
"""Dependency Bloat Reducer — Analyze and reduce dependency bloat in projects"""
import argparse
import json
import os
import sys
...
def analyze_command(args):
    ...
    console.print(f"[bold cyan]Dependency Bloat Reducer[/bold cyan]")  # pragma: no cover
    ...
    if args.json if hasattr(args, "json") else False:  # pragma: no cover
        ...
        table = Table(title="Dependency Bloat Reducer Results")  # pragma: no cover
...
def main():
    parser = argparse.ArgumentParser(description="Dependency Bloat Reducer — Analyze and reduce dependency bloat in projects")
    subparsers = parser.add_subparsers(dest="command", required=True)

    cmd = subparsers.add_parser("analyze", help="Run analyze")
    cmd.add_argument("path", nargs="?", default=".", help="Target path")
    cmd.add_argument("--json", action="store_true", help="Output as JSON")
    cmd.set_defaults(func=analyze_command)
