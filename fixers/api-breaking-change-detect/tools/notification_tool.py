...
def notify_consumers(summary: str, recipients: List[str] = None):
    ...
    """
    if recipients is None:  # pragma: no cover
        recipients = ["dev-team", "api-consumers"]  # pragma: no cover

    # In a real app, integrate with Slack/Email API here.
    # We use print here as a mock.
    print(f"\n[Notification Tool] Notifying {', '.join(recipients)} about changes...")  # pragma: no cover
