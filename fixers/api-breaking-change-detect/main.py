#!/usr/bin/env python3
"""Api Breaking Change Detect — Detect breaking changes in API endpoints by comparing OpenAPI specs"""
import argparse
import json
import os
import sys
...
def detect_command(args):
    ...
    console.print(f"[bold cyan]Api Breaking Change Detect[/bold cyan]")  # pragma: no cover
    ...
    if args.json if hasattr(args, "json") else False:  # pragma: no cover
        ...
        table = Table(title="Api Breaking Change Detect Results")  # pragma: no cover
...
def main():
    parser = argparse.ArgumentParser(description="Api Breaking Change Detect — Detect breaking changes in API endpoints by comparing OpenAPI specs")
    subparsers = parser.add_subparsers(dest="command", required=True)

    cmd = subparsers.add_parser("detect", help="Run detect")
    cmd.add_argument("path", nargs="?", default=".", help="Target path")
    cmd.add_argument("--json", action="store_true", help="Output as JSON")
    cmd.set_defaults(func=detect_command)
