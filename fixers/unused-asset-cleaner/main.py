#!/usr/bin/env python3
"""Unused Asset Cleaner — Find and remove unused assets (images, fonts, etc) from projects"""
import argparse
import json
import os
import sys
...
def scan_command(args):
    ...
    console.print(f"[bold cyan]Unused Asset Cleaner[/bold cyan]")  # pragma: no cover
    ...
    if args.json if hasattr(args, "json") else False:  # pragma: no cover
        ...
        table = Table(title="Unused Asset Cleaner Results")  # pragma: no cover
...
def main():
    parser = argparse.ArgumentParser(description="Unused Asset Cleaner — Find and remove unused assets (images, fonts, etc) from projects")
