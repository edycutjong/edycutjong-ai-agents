#!/usr/bin/env python3
"""Doc Drift Fixer — Detect and fix documentation drift from source code"""
import argparse
import json
import os
import sys
...
def check_command(args):
    ...
    console.print(f"[bold cyan]Doc Drift Fixer[/bold cyan]")  # pragma: no cover
    ...
    if args.json if hasattr(args, "json") else False:  # pragma: no cover
        ...
        table = Table(title="Doc Drift Fixer Results")  # pragma: no cover
...
def main():
    parser = argparse.ArgumentParser(description="Doc Drift Fixer — Detect and fix documentation drift from source code")
    subparsers = parser.add_subparsers(dest="command", required=True)

    cmd = subparsers.add_parser("check", help="Run check")
    cmd.add_argument("path", nargs="?", default=".", help="Target path")
    cmd.add_argument("--json", action="store_true", help="Output as JSON")
    cmd.set_defaults(func=check_command)
