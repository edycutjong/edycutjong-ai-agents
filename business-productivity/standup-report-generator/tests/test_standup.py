...
def test_weekly_summary_limit():
    items = [f"Item {i}" for i in range(1, 16)]
    entry = StandupEntry(author="BusyBee", today=items)
    summary = generate_weekly_summary([entry])
    assert "BusyBee" in summary
    assert "Item 10" in summary
    assert "Item 11" not in summary
    # Check that there are exactly 10 lines starting with "- Item"
    assert summary.count("- Item") == 10
