...
def save_to_file(content: str, filename: str, format: str = "md") -> str:
    ...
    if not filename.endswith(f".{format}"):  # pragma: no cover
        filename = f"{filename}.{format}"  # pragma: no cover

    # Ensure directory exists
    os.makedirs(os.path.dirname(filename) if os.path.dirname(filename) else ".", exist_ok=True)  # pragma: no cover
