#!/usr/bin/env python3
"""Sql To Nosql Migrator — Convert SQL schemas and queries to NoSQL equivalents"""
import argparse
import json
import os
import sys
...
def migrate_command(args):
    ...
    console.print(f"[bold cyan]Sql To Nosql Migrator[/bold cyan]")
    ...
    if args.json if hasattr(args, "json") else False:
        ...
        table = Table(title="Sql To Nosql Migrator Results")
...
def main():
    parser = argparse.ArgumentParser(description="Sql To Nosql Migrator — Convert SQL schemas and queries to NoSQL equivalents")
    subparsers = parser.add_subparsers(dest="command", required=True)

    cmd = subparsers.add_parser("migrate", help="Run migrate")
    cmd.add_argument("path", nargs="?", default=".", help="Target path")
    cmd.add_argument("--json", action="store_true", help="Output as JSON")
    cmd.set_defaults(func=migrate_command)
