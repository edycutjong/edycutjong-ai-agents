...
def cmd_convert(args):
    text = sys.stdin.read() if args.file == "-" else open(args.file).read()  # pragma: no cover
    fmt = detect_format(text)  # pragma: no cover
    if fmt == "json": print(convert_json_string_to_yaml(text))  # pragma: no cover
    else: print(convert_yaml_string_to_json(text))  # pragma: no cover
def main():
