#!/usr/bin/env python3
"""Database Diagram Generator — Generate ER diagrams from database schemas"""
import argparse
import json
import os
import sys
...
def generate_command(args):
    ...
    console.print(f"[bold cyan]Database Diagram Generator[/bold cyan]")  # pragma: no cover
    ...
    if args.json if hasattr(args, "json") else False:  # pragma: no cover
        ...
        table = Table(title="Database Diagram Generator Results")  # pragma: no cover
...
def main():
    parser = argparse.ArgumentParser(description="Database Diagram Generator — Generate ER diagrams from database schemas")
